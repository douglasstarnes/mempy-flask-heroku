...
To start, let's examine the `todo_item` documents in the database.  Run the `mongo` shell in the terminal:

```
> use mempydemo
switched to db mempydemo
> db.todo_item.find();
{ "_id" : ObjectId("55180985be0cfc1ca3532e0f"), "task" : "Very important", "due_date" : ISODate("2015-04-05T14:17:41.353Z"), "priority" : 10, "complete" : false }
{ "_id" : ObjectId("55180a13be0cfc1ca3532e10"), "task" : "Overdue ", "due_date" : ISODate("2015-03-28T14:20:03.730Z"), "priority" : 2, "complete" : false }
{ "_id" : ObjectId("55180a23be0cfc1ca3532e11"), "task" : "Due today", "due_date" : ISODate("2015-03-29T14:20:19.097Z"), "priority" : 5, "complete" : false }
{ "_id" : ObjectId("55180a30be0cfc1ca3532e12"), "task" : "Nothing special", "due_date" : ISODate("2015-04-12T14:20:32.097Z"), "priority" : 5, "complete" : false }
```

Every document has an '_id' property that is automatically generated by MongoDB so it is guaranteed to be unique.  We can use this property to identify a `TodoItem` that we want to mark as complete.  First, let's add a handler function to the application that will find a `TodoItem` with an id, and the update it's `complete` property:

```python
@app.route('/mark_complete')
def mark_complete():
    the_id = request.args.get('id')
    TodoItem.objects(pk=the_id).update_one(set__complete=True)
    return redirect('/')
```

When we used a form to capture user input, we did so with a POST request.  For this handler, we'll use a GET request.  The URL will be in the form of `/markcomplete?id=<id>`.  The `request` object GET values are stored in the `args` property which has a `get()` method that takes the key of the GET value to find.  The next line gets all of the `TodoItem` documents, filtering them by the primary key or 'pk'.  Since we anticipate this returning only one row, we can call `update_one()` on the result.  The keyword argument `set__complete` warrants some explanation.  This is actually shorthand.  The are several actions that `update_one()` can do, with `set` being one of them.  The value we want to update is `complete`.  So `set__complete` says set the complete property to the value on the right hand side of the keyword argument which in this case is `True`.  That will also save the change to the database.  Then we'll redirect the browser to the index and the task will no longer be in the list.

To call `mark_complete` function, we'll add a new column to the list that contains a button. (Actually a link formatted as a button by Bootstrap.)
...
The 'href' creates a link that refers to the `mark_complete` route with the id for each `TodoItem`.  The 'class' is what tells Bootstrap to render this link as a button.  Here's how it looks in the browser.

![Complete tasks](readme_images/completetasks.png)

As you hover over the _Complete_ buttons, notice that the URL for each one has a different id.  Let's take care of that overdue task.  Click on the second _Complete_ button.

![After marking](readme_images/aftermarking.png)

Great!  We're all caught up!  To see the change made in the database, use the `mongo` shell in the terminal:
...
> db.todo_item.find();
{ "_id" : ObjectId("55180985be0cfc1ca3532e0f"), "task" : "Very important", "due_date" : ISODate("2015-04-05T14:17:41.353Z"), "priority" : 10, "complete" : false }
{ "_id" : ObjectId("55180a13be0cfc1ca3532e10"), "task" : "Overdue ", "due_date" : ISODate("2015-03-28T14:20:03.730Z"), "priority" : 2, "complete" : true }
{ "_id" : ObjectId("55180a23be0cfc1ca3532e11"), "task" : "Due today", "due_date" : ISODate("2015-03-29T14:20:19.097Z"), "priority" : 5, "complete" : false }
{ "_id" : ObjectId("55180a30be0cfc1ca3532e12"), "task" : "Nothing special", "due_date" : ISODate("2015-04-12T14:20:32.097Z"), "priority" : 5, "complete" : false }
```
